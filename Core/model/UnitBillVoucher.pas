...
uses
  ...
  UnitIBillEx, XPMenu;
type
  TFrmBillVoucher = class(TFrmModel,IBillEx)
    Panel1: TPanel;
    Splitter2: TSplitter;
    ...
    PnlBtm: TPanel;
    ShapeBtmFrame: TShape;
    ShapeBtmLine: TShape;
    ActConfigBtm: TAction;
    ...
    procedure ActConfigBtmExecute(Sender: TObject);
    procedure ShapeBtmFrameMouseDown(Sender: TObject; Button: TMouseButton;
...
var
  FrmBillVoucher: TFrmBillVoucher;

implementation
        uses   Editor, UnitBillEx,  UnitCreateComponent,UnitUserDefineRpt,UnitUserReport,UPublicFunction,UnitFrmAnalyserEx ,UnitCtrlConfig ,UnitMDLookupImport ;
{$R *.dfm}

procedure TFrmBillVoucher.InitFrm(frmid: string);
var i:integer;
begin
//ini toolbtns
//ini mtdataset
...
procedure TFrmBillVoucher.OpenBill(BillCode: string);
var  i:Integer;
begin
...
procedure TFrmBillVoucher.OpenCloseAfter(IsOpened:Boolean);
var i:integer;
begin
...
procedure TFrmBillVoucher.SetCtrlStyle(fEnabled:Boolean);
 var bkColor:TColor;
begin
...
procedure TFrmBillVoucher.SetRitBtn;
...
procedure TFrmBillVoucher.SetCheckStyle(IsChecked:Boolean);
begin
...
procedure TFrmBillVoucher.SetBillStyle(fReadOnly:Boolean);
begin
...
procedure TFrmBillVoucher.CloseAction1Execute(Sender: TObject);
begin
self.Close;
end;


procedure TFrmBillVoucher.ScrollTopDblClick(Sender: TObject);
...
procedure TFrmBillVoucher.ScrollBtmDblClick(Sender: TObject);
var CrtCom:TfrmCreateComponent    ;
begin
// modeltpe:=Bill;
  if LoginInfo.Sys  then  begin
...
procedure TFrmBillVoucher.DBGridDLDblClick(Sender: TObject);
var CrtCom:TfrmCreateComponent    ;
begin
// modeltpe:=Bill;
  if LoginInfo.Sys  then  begin
...
procedure TFrmBillVoucher.dlDataSet1CalcFields(DataSet: TDataSet);
begin
...
procedure TFrmBillVoucher.CancelAction1Execute(Sender: TObject);
begin 
...
procedure TFrmBillVoucher.CloseBill;
begin
   mtDataSet1.Close;
   dlDataSet1.Close;
   OpenCloseAfter(false);
end;
procedure TFrmBillVoucher.UpdateSysFieldsAndCalValue;
var  sql:string;
var i:integer;
begin
...
procedure TFrmBillVoucher.Ds_UpdateAllRecs(fGrid:TDBgrid;fFields,fValues:Variant;PPgBarSave:Tprogressbar;DelOnCalEvent:boolean=true);
var bk:Pointer;
var i:integer;
...
procedure TFrmBillVoucher.SaveAction1Execute(Sender: TObject);
...
procedure TFrmBillVoucher.EditPostAfter(IsEnabled:Boolean);
begin
    if IsEnabled then
       SetRitBtn
    else
...
procedure TFrmBillVoucher.ImportAction1Execute(Sender: TObject);
begin
...
procedure TFrmBillVoucher.dlDataSet1AfterPost(DataSet: TDataSet);
begin
...
procedure TFrmBillVoucher.DlDataSource1StateChange(Sender: TObject);
begin
...
procedure TFrmBillVoucher.mtDataSource1StateChange(Sender: TObject);
begin
  if (MtDataSource1.State=dsEdit) or (MtDataSource1.State=dsInsert) then
     EditPostAfter(false);
end;

procedure TFrmBillVoucher.CheckAction1Execute(Sender: TObject);
begin
...
procedure TFrmBillVoucher.ActInfoExecute(Sender: TObject);
begin
...
procedure TFrmBillVoucher.FirstAction1Execute(Sender: TObject);
begin
...
procedure TFrmBillVoucher.PriorAction1Execute(Sender: TObject);
begin
  if (F_ParamData<>nil) and ( F_ParamData.Active )then
  begin
      F_ParamData.Prior ;
...
procedure TFrmBillVoucher.NextAction1Execute(Sender: TObject);
begin
  if (F_ParamData<>nil) and ( F_ParamData.Active )then
  begin
      F_ParamData.Next ;
...
procedure TFrmBillVoucher.LastAction1Execute(Sender: TObject);
begin
  if (F_ParamData<>nil) and ( F_ParamData.Active )then
  begin
      F_ParamData.Last  ;
...
procedure TFrmBillVoucher.SetParamDataset(PDataset: Tdataset);
begin
self.F_ParamData :=PDataset;
end;
{
procedure TFrmBillVoucher.mtDataSet1AfterOpen(DataSet: TDataSet);
begin
...
procedure TFrmBillVoucher.ConfigComplete(P_FlagFldName,
...
procedure TFrmBillVoucher.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmBillVoucher.AppendAction1Execute(Sender: TObject);
begin
if    not (dlDataSet1.LockType = ltReadOnly) then
 dlDataSet1.Append;
end;

procedure TFrmBillVoucher.DBGridDLExit(Sender: TObject);
begin
    if self.dlDataSet1.State in [dsinsert,dsedit] then
    begin
...
procedure TFrmBillVoucher.NewAction1Execute(Sender: TObject);
begin
...
procedure TFrmBillVoucher.DeleteAction1Execute(Sender: TObject);
begin
    if    not (dlDataSet1.LockType = ltReadOnly) then
    begin
...
procedure TFrmBillVoucher.dlDataSet1AfterScroll(DataSet: TDataSet);
begin
     statLabel1.Caption:=intTostr(dlDataSet1.RecNo)+'/'+intTostr(dlDataSet1.RecordCount);
end;

procedure TFrmBillVoucher.ActSaveWithOutDLExecute(Sender: TObject);
var  s,ss,f:widestring;
//var  fDict:TBeforePostDict;
//var  pKeys:Tstrings;
var i:integer;
...
procedure TFrmBillVoucher.Action1Execute(Sender: TObject);
...
procedure TFrmBillVoucher.ActRemMchSvPayExecute(Sender: TObject);
begin
...
procedure TFrmBillVoucher.ActlocateExecute(Sender: TObject);
var i:integer;
begin
{
...
procedure TFrmBillVoucher.CmbFldsChange(Sender: TObject);
var i:integer;
begin
   {
...
procedure TFrmBillVoucher.EdtValuesKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
  var LstChkState:Tstrings;
begin
{
...
procedure TFrmBillVoucher.ActDeleteExecute(Sender: TObject);
begin
...
procedure TFrmBillVoucher.BtnRecLocateClick(Sender: TObject);
begin
//PnlFunction.Visible := BtnRecLocate.Down;
end;

procedure TFrmBillVoucher.ActShowHeadExecute(Sender: TObject);
begin
//self.ScrollTop.Visible :=self.BtnShowHead.Down;
end;

procedure TFrmBillVoucher.PnlBtmDblClick(Sender: TObject);
var CrtCom:TfrmCreateComponent    ;
...
procedure TFrmBillVoucher.FormCreate(Sender: TObject);
begin
...
procedure TFrmBillVoucher.DBLClick(sender: Tobject);
var CrtCom:TfrmCreateComponent    ;
begin
    // modeltpe:=Bill;
    if LoginInfo.Sys  then  begin
...
procedure TFrmBillVoucher.RemoveAction1Execute(Sender: TObject);
...
procedure TFrmBillVoucher.ActEditExecute(Sender: TObject);
begin
...
procedure TFrmBillVoucher.ActSetBitExecute(Sender: TObject);
begin
  inherited;
...
procedure TFrmBillVoucher.PrintAction1Execute(Sender: TObject);
 var
    printID:string;
...
procedure TFrmBillVoucher.ActSaveExecDLProcExecute(Sender: TObject);
var
  fbk:TBookmark;
  i:integer;
begin
...
procedure TFrmBillVoucher.ActUncheckExecute(Sender: TObject);
begin
  inherited;
    if self.mtDataSet1.IsEmpty then exit;
    Screen.Cursor:=CrSqlWait;
...
procedure TFrmBillVoucher.RefreshAction1Execute(Sender: TObject);
begin
  inherited;
 mtDataSet1.Close;
 mtDataSet1.Open;
...
procedure TFrmBillVoucher.ActSaveHaveTextFomulaExecute(Sender: TObject);
var  s,ss,f:widestring;
//var  fDict:TBeforePostDict;
var  pKeys,DLKeyFLDs:Tstrings;
var i:integer;
...
procedure TFrmBillVoucher.ActChkChgExecute(Sender: TObject);
begin
  inherited;
     EditPostAfter(false);
...
procedure TFrmBillVoucher.UserLogExecute(Sender: TObject);
begin
...
procedure TFrmBillVoucher.ActPropertyExecute(Sender: TObject);
var sql:string;
var
  frmid:string;
  fBillType:string;
...
procedure TFrmBillVoucher.btnCtrlClick(Sender: TObject);
...
function TFrmBillVoucher.FirstCommarSection(Pstr: string): string;
begin
  if Pos(',',Pstr)<>0 then
...
procedure TFrmBillVoucher.InActiveBillExecute(Sender: TObject);
begin
  inherited;
...
procedure TFrmBillVoucher.ActiveBillExecute(Sender: TObject);
begin
   inherited;
  if (self.dlDataSet1.FindField('FIsCls')<>nil) then
  begin 
...
procedure TFrmBillVoucher.actMDLookupExecute(Sender: TObject);
...
procedure TFrmBillVoucher.ActSaveHaveTextFomulaZeroQtyExecute(Sender: TObject);
var  s,ss,f:widestring;
//var  fDict:TBeforePostDict;
var  pKeys,DLKeyFLDs:Tstrings;
var i:integer;
...
procedure TFrmBillVoucher.ActApportionExecute(Sender: TObject);
var i:integer;
...
procedure TFrmBillVoucher.ActConfigBtmExecute(Sender: TObject);
begin
    PnlBtmDblClick(sender);
end;

procedure TFrmBillVoucher.ShapeBtmFrameMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
    if ( ssShift in  shift  ) and (logininfo.Sys)then
    begin
         PnlBtmDblClick(sender);
    end
end;
