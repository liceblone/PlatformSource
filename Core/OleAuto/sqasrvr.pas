        {*************************************************************************
        *
        *    Copyright (c) 1996 - SQA Inc., All Rights Reserved
        *
        *    Name:              DELINT.H
        *    Description:       Internal Header File for Delphi Extension Library
        *
        *    Revision History:
        ...
        **************************************************************************}
        ...
        uses Forms, Classes, Controls, SysUtils;
       ...
       { This is called in the initialization section of the unit. This adds the application
        object dispatch pointer to a window property and hence enables communication of the AUT
        with Delphi Extension. }
        ...
        var
                ...
                V : ^Variant;
        begin
                propName := PChar('SQAApplicationObject');
                ...
                // (ONLY) one variant and an application object would be hanging around.
                // The two objects should be cleaned when AUT shuts down, but
                // some debug code could trigger assertion because of reference count.
                SetProp( Application.Handle, propName, THandle( V ) );
        end;

        { Someday this needs to be called from some AUT cleanup code automatically. }
        ...
        begin
                ...
                th := Windows.GetProp( Application.Handle, propName );
                if ( th <> 0 ) then
                begin
                        V := PVariant(th);
                        Dispose( V );
                        //VarToInterface(V^).Release;
                        Windows.RemoveProp( Application.Handle, propName );
                end;
        end;
